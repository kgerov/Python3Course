...
def is_match(partner1, partner2):
    if len(set(partner1['interests']).intersection(set(partner2['interests']))) <= 0:
        return False
    if abs(int(partner1['age']) - int(partner2['age'])) > 6:
        return False
    if partner2['name'] in partner1['ex']:
        return False
...
for i, partner1 in enumerate(people):
    for j, partner2 in enumerate(people):
        if i != j and is_match(partner1, partner2):
            print("{0} ({1}) и {2} ({3}) ; общи интереси: "
                  "{4}".format(partner1['name'],partner1['age'],
                               partner2['name'], partner2['age'],
                               ", ".join(set(partner1['interests']).intersection(set(partner2['interests'])))))